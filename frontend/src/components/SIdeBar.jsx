...
const SIdeBar = ({isMenuOpen, setIsMenuOpen}) => {
    ...
    const {chats, setSelectedChats, theme, setTheme, user, navigate, createNewChat, setChats, axios, fetchUsersChats, setToken, token} = useAppContext()
  ...
  return (
    ...
    {/* New Chat Button */}
    <button onClick={createNewChat}  className='flex justify-center items-center bg-gradient-to-r from-[#A456F7] to-[#3D81F6] w-full py-2 mt-10 text-white text-sm rounded-md cursor-pointer'>
    ...
    <div className='flex-1 overflow-y-scroll mt-3 text-sm space-y-3'>
        {
            chats.filter((chat ) => 
                chat.messages[0] ? chat.messages[0] ?. content.toLowerCase().includes(search.toLowerCase()) : chat.name.toLowerCase().includes(search.toLowerCase())).map((chat) => {

                    return <div onClick={() => {navigate('/'); setSelectedChats(chat); setIsMenuOpen(false)}} key={chat._id} className='p-2 px-4 dark:bg-[#57317C]/10 border border-gray-300 dark:border-[#80609F]/15 rounded-md cursor-pointer flex justify-between group'>
    ...
    {/* community images */}
    <div onClick={() => {navigate('/community'); setIsMenuOpen(false)}} className='flex items-center gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all duration-200'>
    ...
    <div onClick={() => {navigate('/credits'); setIsMenuOpen(false)}} className='flex items-center gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all duration-200'>
    ...
    {/* User Account */}
     <div  className='flex items-center gap-3 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md group'>
        <img src={assets.user_icon} alt="" className='w-7 not-dark:invert' />
